...
export const CommandPalette: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4">
      ...
      {/* Palette Box */}
      <div className="relative w-full max-w-xl bg-surface-300 border border-border rounded-xl shadow-modal z-10 overflow-hidden flex flex-col animate-slide-down">
        {/* Search input */}
        <div className="flex items-center px-4 py-3 border-b border-border-subtle bg-surface-200/80">
          ...
          <input
            ...
            placeholder="Type a command, search members, or jump to view..."
            className="w-full bg-transparent text-sm text-zinc-100 placeholder:text-zinc-500 focus:outline-none"
          />
          <kbd className="text-[10px] font-mono text-zinc-400 bg-surface-100 border border-border-subtle px-1.5 py-0.5 rounded shadow-xs">
            ESC
          </kbd>
        </div>
                ...
                {filteredMembers.slice(0, 6).map((member) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-7 h-7 rounded-full bg-surface-100 border border-border-subtle overflow-hidden shrink-0 flex items-center justify-center text-xs font-semibold text-zinc-300">
                        {member.avatarUrl ? (
                ...
                {filteredQuickActions.map((action) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="p-1 rounded bg-surface-200 border border-border-subtle">
                        {action.icon}
                      </div>
          ...
          {query.trim() &&
            ...
            filteredNavActions.length === 0 && (
              <div className="p-8 text-center text-zinc-400">
                ...
                <p className="text-sm font-medium text-zinc-300">No matching commands or members</p>
                <p className="text-xs text-zinc-500 mt-1">
                  Try searching by member name, phone number, or action.
                </p>
              </div>
            )}
        </div>
        ...
        {/* Footer info */}
        <div className="px-4 py-2 border-t border-border-subtle bg-surface-200/40 text-[11px] text-zinc-400 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span>Navigation: <kbd className="font-mono bg-surface-100 px-1 py-0.5 rounded border border-border-subtle text-[10px]">↑</kbd> <kbd className="font-mono bg-surface-100 px-1 py-0.5 rounded border border-border-subtle text-[10px]">↓</kbd></span>
            <span>Select: <kbd className="font-mono bg-surface-100 px-1 py-0.5 rounded border border-border-subtle text-[10px]">↵</kbd></span>
          </div>
          <span className="text-brand-400 font-medium">GYM OS v1.0</span>
        </div>
      </div>
    </div>
  );
